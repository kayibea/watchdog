...
static int l_add(lua_State *L) {
  ...
  if (watch_fd < 0)
    return luaL_error(L, "inotify_add_watch failed: %s", strerror(errno));
...
static int l_remove(lua_State *L) {
  ...
  if (inotify_rm_watch(wd->fd, watch_fd) < 0) {
    return luaL_error(L, "inotify_rm_watch failed: %s", strerror(errno));
  }
...
static int l_init(lua_State *L) {
  ...
  if (fd < 0) return luaL_error(L, "inotify_init1 failed: %s", strerror(errno));
